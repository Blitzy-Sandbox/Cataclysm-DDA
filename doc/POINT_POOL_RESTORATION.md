...
patch fails ("patch does not apply"). The feature is therefore restored
...
Per the project's user-specified **Explainability** rule, all non-trivial
implementation decisions and their rationale live in this document (the tables
below) rather than in code comments. Section A is the decision log; Section B is
the bidirectional traceability matrix mapping every removed construct to its
